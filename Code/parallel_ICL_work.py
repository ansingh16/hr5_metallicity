...
files = glob.glob('*.dat')
snaps=[(int(file.split('.')[0])) for file in sorted(files,reverse=True)]
# with concurrent.futures.ProcessPoolExecutor(8) as p:
#     p.map(read_snapfile,snaps)

Parallel(n_jobs=5)(delayed(read_snapfile)(snap) for snap in sorted(snaps,reverse=True))
