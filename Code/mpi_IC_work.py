...
def read_snapfile(clusfile):

    snapno=int(clusfile.split('.')[0])    
                        ...
                        if clusters['HostHaloID'].iloc[kkk]==hline:
                                        ...
                                        for i in range(thalo['nsub']):
                                                ...
                                                for dat in ['posstar','posdm','posgas','velstar',\
                                                    'veldm','velgas','massstar','massdm','massgas','tgas','zgas','zstar','ogas','fegas','hgas']:

                                                    if f'{hline}/{sline}/{dat}' in f:
                                                        del f[f'{hline}/{sline}/{dat}']
    ...
    with open(f'Done{snap}.done',w) as tt:
...
def main():
    ...
    print('M here')
    if rank == 0:
        # The master process reads the list of file names
    ...
    # Divide the file list among processes
    files_per_process = len(file_list) // size
    remainder = len(file_list) % size

    if rank < remainder:
        file_chunk = file_list[rank * (files_per_process + 1):(rank + 1) * (files_per_process + 1)]
    else:
        file_chunk = file_list[rank * files_per_process + remainder:(rank + 1) * files_per_process + remainder]

    # Read files in parallel
    for filename in file_chunk:

        print(f"Process {rank} reading file {filename}")
        read_snapfile(filename)
